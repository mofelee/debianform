...
assert_remote "state records stopped component service desired state" \
  "grep -F 'host.cihost.components.service_unit_fixture.services.service[\\\"dbf-raw\\\"]' /var/lib/debianform-integration/service-unit-state.json && grep -F 'host.cihost.components.service_unit_fixture.services.service[\\\"dbf-structured\\\"]' /var/lib/debianform-integration/service-unit-state.json"
