if [[ "$EXPECTED_DISTRIBUTION" == "ubuntu" ]]; then
  ...
  log "verifying stock Netplan ownership rejects generic, raw-content, and raw-source networkd plan"
  ...
  for evidence in \
    ...
    'host.cihost.systemd.networkd.network["90-dbf-netplan-generic"]' \
    'host.cihost.systemd.networkd.network["91-dbf-netplan-raw-content"]' \
    'host.cihost.systemd.networkd.netdev["92-dbf-netplan-raw-source"]' \
  ...
  assert_remote "Netplan conflict caused no network file, service, or state-resource mutation" \
    "set -eu; test ! -e /etc/systemd/network/90-dbf-netplan-generic.network; test ! -e /etc/systemd/network/91-dbf-netplan-raw-content.network; test ! -e /etc/systemd/network/92-dbf-netplan-raw-source.netdev; test ! -e /var/lib/debianform-integration/netplan-preflight-state.json || grep -F '\"resources\": {}' /var/lib/debianform-integration/netplan-preflight-state.json; systemctl is-active systemd-networkd.service > /tmp/debianform-networkd-active.current 2>&1 || true; systemctl is-enabled systemd-networkd.service > /tmp/debianform-networkd-enabled.current 2>&1 || true; cmp -s /tmp/debianform-networkd-active /tmp/debianform-networkd-active.current; cmp -s /tmp/debianform-networkd-enabled /tmp/debianform-networkd-enabled.current; rm -f /tmp/debianform-networkd-active.current /tmp/debianform-networkd-enabled.current"
