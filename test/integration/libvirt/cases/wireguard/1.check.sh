...
assert_remote wg-a "wg-a networkd netdev exists" \
  "test -e /etc/systemd/network/10-wg0.netdev && grep -F 'PrivateKeyFile=/etc/wireguard/private.key' /etc/systemd/network/10-wg0.netdev && grep -F 'RouteTable=off' /etc/systemd/network/10-wg0.netdev"
