...
assert_remote "compose project was converged back to running after manual stop" \
  "docker compose -p app -f /opt/debianform-compose-app/compose.yaml ps --format json | grep -i 'running'"
assert_remote "generated compose systemd unit remains active and enabled after repair" \
