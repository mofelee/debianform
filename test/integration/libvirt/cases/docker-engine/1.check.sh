...
assert_remote "docker engine state records high-level docker resources and runtime facts" \
  "grep -F 'host.cihost.docker.package[\\\"docker-ce\\\"]' /var/lib/debianform-integration/docker-engine-state.json && grep -F 'host.cihost.docker.service[\\\"docker\\\"]' /var/lib/debianform-integration/docker-engine-state.json && grep -F '\"codename\": \"${DBF_INTEGRATION_TARGET_CODENAME}\"' /var/lib/debianform-integration/docker-engine-state.json"
