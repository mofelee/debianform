assert_remote "docker official repository source exists" \
  "grep -F 'URIs: https://download.docker.com/linux/debian' /etc/apt/sources.list.d/docker_official.sources && grep -F 'Suites: trixie' /etc/apt/sources.list.d/docker_official.sources"
...
assert_remote "docker engine state records high-level docker resources and runtime facts" \
  "grep -F 'host.cihost.docker.package[\\\"docker-ce\\\"]' /var/lib/debianform-integration/docker-engine-state.json && grep -F 'host.cihost.docker.service[\\\"docker\\\"]' /var/lib/debianform-integration/docker-engine-state.json && grep -F '\"codename\": \"trixie\"' /var/lib/debianform-integration/docker-engine-state.json"
