...
| APT 仓库 | `debian_apt_repository` | P1 | `ansible.builtin.deb822_repository` |
...
- `key`：可选 object，支持 `url`、`content`、`source`、`path`。
...
- `key.content` 或 `key.source` 来自本地时，不触发远端下载，也不自动要求
  `ca-certificates`。
