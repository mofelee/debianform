...
| APT 源 | `debian_apt_source` | P1 | `ansible.builtin.deb822_repository` |
...
| `update_cache` | 安装前执行 `apt-get update` |
| `cache_valid_time` | 缓存超过指定秒数才更新 |
...
### 6.2 `debian_apt_source`

**状态：** P1。

**类型：** 原生文件优先的声明资源。

**底层：** `/etc/apt/sources.list.d/*.sources`、deb822、`apt-get update`。

Debian 13 应优先使用 deb822 `.sources`，不新增对废弃 `apt-key` 工作流的抽象。

```hcl
debian_apt_source "docker" {
  host = "server1"
  name = "docker"
...
- `signed_by`：只接受绝对 keyring 路径或内嵌 key。
- `enabled`、`ensure`、`update_cache`。

应用要求：

- 结构化字段必须序列化为稳定顺序的 deb822。
- 修改后先验证语法，再按需更新 cache。
- `trusted = true`、`allow_insecure = true` 一类选项默认不提供；确有需求时必须明显标注风险。

### 6.3 `debian_apt_preferences_file`
