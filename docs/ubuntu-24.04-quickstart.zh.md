...
Ubuntu 26.04 LTS amd64 是另一条独立验证的 Preview 路径；本页结果不能替代 26.04 证据。当前
Ubuntu 支持范围不包括 22.04、arm64、桌面环境、Snap、PPA、Ubuntu Pro、cloud-init 生命周期、
sudo/become 或默认 `ubuntu` 用户连接。DebianForm 继续使用同一个 `dbf` CLI、`*.dbf.hcl` DSL
和 state，不存在 UbuntuForm。
...
- Ubuntu 24.04 LTS amd64 当前有独立的 22-case 阻断矩阵，但 Preview 仍表示用户反馈和 release
  验证积累少于 Beta。
...
[Ubuntu 24.04 支持契约](ubuntu-24.04-support-contract.zh.md)。Ubuntu 26.04 用户应从
[Ubuntu 26.04 Preview Quickstart](ubuntu-26.04-quickstart.zh.md)开始。
