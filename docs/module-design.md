...
```text
ensure package ca-certificates
  -> fetch https://pkg.labs.nic.cz/gpg
...
如果 key 由本地 `file()` 或内嵌 `content` 提供，则不需要远端下载步骤，也不应自动
安装 `ca-certificates`：
...
- 通过 HTTPS 远端获取 APT key 时，provider 默认依赖 `ca-certificates`。
...
当 `debian_apt_repository` 支持从远端 URL 获取 signing key 时，必须按语义依赖建图：

```text
package:host:ca-certificates
  -> tls-ca:host
  -> fetch_apt_key[host, https-url]
  -> path:host:/etc/apt/keyrings/<name>.asc
  -> apt-repository:host:<name>
  -> apt_update[host]
```

规则：

- `key.url` 使用 `https://` 时，provider 自动要求 `tls-ca:host`。
- `tls-ca:host` 默认由内部的 `ca-certificates` 包节点提供。
- `key.url` 使用 `http://` 时不需要 `tls-ca:host`，但仍需要下载动作节点。
- `key.content` 或 `content = file(...)` 不触发远端下载，也不触发 `ca-certificates`。
- 下载工具是 provider 实现细节；如果实现依赖 `curl`、`wget` 或其他包，也应由 provider 自动声明依赖，不暴露给用户。
- `ca-certificates` 安装使用目标主机现有 APT 配置；如果目标机连当前 Debian 仓库都无法访问，plan/apply 应返回清晰错误，而不是要求用户手写顺序。
