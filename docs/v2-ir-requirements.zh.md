...
```text
DSL 层
  用户写的 HCL：host、profile、resource、kernel、packages、services 等。

合并层
  解析 imports、执行 merge modifier、得到每台 host 的最终领域配置。

中间表达层
  归一化后的 HostSpec 和 ResourceInstanceSpec，仍然表达主机目标状态。
...
type Program struct {
    Hosts     []HostSpec
    Resources map[string]ResourceTemplateSpec
}
```
...
type HostSpec struct {
    ...
    Resources []ResourceInstanceSpec
}
```
...
DebianForm 名称，不能让 resource compiler 直接处理 `x86_64`、`aarch64`、
Debian architecture 和 Go architecture 的多套别名。

如果 DSL 没有显式 architecture，离线生成的 HostSpec 可以暂时标记为
`ArchitectureUnknown`；连接目标并探测后，必须在 source 选择和 ResourceGraph
编译前完成归一化。
...
顶层 `resource` 是部署组件模板，不是 provider resource，也不是可独立 apply 的对象。
...
- `binary`、`archive`、`file`、`ca_certificate` 各自使用独立 schema 校验，不能把
  `type` 当成完全动态的弱类型 map。
- resource 的内部领域对象保留 resource 来源地址，不能伪装成 host 自身声明。
- resource 实例不能直接修改 host 的 `ssh`、`state` 或 profile merge 结果。

resource 实例的中间地址示例：

```text
host.server1.resources.rclone.download
host.server1.resources.rclone.install["/usr/local/bin/rclone"]
host.server1.resources.myapp.users.user["myapp"]
host.server1.resources.myapp.files.file["/etc/myapp/config.yaml"]
host.server1.resources.myapp.systemd.service["myapp.service"]
```

resource 内部依赖在编译时按领域语义推导：
...
如果 resource 与 host/profile 或另一个 resource 生成相同远端 identity，例如同一
文件路径、用户、group 或 systemd unit，编译前必须报冲突。resource 声明顺序不能
作为覆盖优先级。
...
- 展开领域配置到低阶资源。
- 展开每个 host 挂载的 `ResourceInstanceSpec`。
- 生成稳定资源地址。
- 生成显式依赖边。
- 生成语义依赖边。
- 保留 source address。
...
- sysctl key 不能为空。
- kernel module 名不能为空。
- host 引用的 resource 必须存在且不能重复实例化。
- host 架构必须能选择唯一 resource source。
- resource 的 URL、SHA-256、安装路径和解压参数必须通过对应类型校验。
- host/profile/resource 之间不能产生远端 identity 冲突。
...
```text
host/profile/resource DSL -> HostSpec -> ResourceGraph -> plan/apply
```
