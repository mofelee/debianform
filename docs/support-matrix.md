...
| `test/integration/libvirt/cases/component-multi-instance-names` | Beta | Two mounts of one component with per-instance systemd unit, service, directory, and file names; no-op convergence and cleanup. |
...
| `test/integration/libvirt/cases/*` | Beta/Preview | 24 Beta cases each on Debian 12/13 amd64 and 24 Preview cases each on Ubuntu 24.04/26.04 amd64, covering platform assertions, validate, online plan, drift check, apply, JSON no-op plan, check, and case-specific assertions. |
