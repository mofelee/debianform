...
DebianForm 需要在用户 DSL 和最终低阶资源之间增加一个明确的中间表达。
...
`Hostname` 默认等于 host label。`Architecture` 和 `Codename` 是运行时 facts：
在线 `plan`/`check`/`apply` 连接目标后探测，并在 component 表达式求值、source 选择和
ResourceGraph 编译前注入 HostSpec。`Architecture` 统一使用 `amd64`、`arm64` 等
DebianForm 名称，不能让 component compiler 直接处理 `x86_64`、`aarch64`、
Debian architecture 和 Go architecture 的多套别名。`Codename` 使用 Debian
release codename，例如 `bookworm` 或 `trixie`。如果 DSL 显式声明
architecture/codename，探测值必须与声明值一致。
...
- host 没有显式 architecture 时，由在线 facts discovery 探测；`plan --offline` 只能完整
  处理不依赖 runtime facts 的 component。
...
current 不需要兼容旧 state address。第一版建议 state 优先记录中间地址，并保留低阶
...
current 是破坏式重设计，不要求渐进式迁移，也不要求旧 `低阶 provider 资源` 配置继续作为用户语法
存在。

current 拥有独立的 provider 执行路径。旧实现不作为 的运行时依赖，也不能决定 的用户模型。
...
中间表达首先是内部编译边界，用于让 语义清晰、可验证、可解释。
