<p align="right"><a href="requirements.md">English</a> | <strong>简体中文</strong></p>

# DebianForm 需求文档

## 背景

DebianForm 是当前主线，可以采用破坏式重设计。

current 的目标不是在旧实验语法上继续堆功能，而是重新设计一层面向用户的系统配置 DSL，
...
- 不要求兼容旧实验配置语法，允许重新设计 state 地址和资源地址。
...
- 可以重新设计 state address，不需要兼容旧 state address。
...
plan、check 和 apply 必须基于三类状态输入，而不能只比较当前配置和上次写入的
state：
...
- host 内的 component instance label 在同一 host 内唯一，并进入 地址。
...
- 第一阶段不提供 `remove` 字段，避免把“期望缺失”和“从管辖集合移除”混在一起。
- list 合并默认 append 去重。
- 包名必须非空。
...
current 的主模型应直接暴露 nftables，而不是把防火墙抽象成
...
完整示例见
[examples/nftables.dbf.hcl](../../../examples/nftables.dbf.hcl)。
...
详细设计见 [ir-requirements.zh.md](ir-requirements.zh.md)。
...
- `OperationNode` 是 ResourceGraph 中的一等 DAG 节点，有稳定 地址。
...
Plan 应以用户能理解的 地址为主，但不能只输出扁平资源列表。current 的变更经常发生在
...
JSON 格式详见 [plan-format.zh.md](../../plan-format.zh.md)。
...
- `plan` 先探测 runtime facts、读取 state 和 observed，再展示按 地址组织的计划；
  不写 state，不执行变更。
...
- `fleet` 只作为组合压力示例，不应成为所有单元测试的唯一输入。
