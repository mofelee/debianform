...
- [x] 核心闭环已经完整：validate、plan、apply、check、state、lock、observed drift 和 SSH 执行路径都存在。
...
- [x] 可运行示例已在 README 中列出。
- [x] `examples/fleet.dbf.hcl` 已标记为 design-only fixture。
...
- [x] CLI 主路径：`validate`、`fmt`、`plan`、`apply`、`check`。
- [x] 辅助 CLI：`version`、`component inspect`、`variable inspect`。
- [x] parser 支持顶层结构和领域 block。
...
- [x] runnable examples validate 测试。
...
- [x] `dbf` CLI、state 和 SSH lock 实现口径审阅。
...
- [x] 新增 `docs/support-matrix.zh.md`，覆盖 CLI 平台、目标主机、CLI 命令、顶层
...
- [x] 新增 `examples/realistic-systemd-app.dbf.hcl`，覆盖低权限 systemd app 的
...
- [x] 核对 release process、release notes template、state 和 plan format 现有口径。
...
- [x] README、release process、state、plan format 和 support matrix 增加入口。
...
- [x] 核对 SECURITY、README 权限模型、requirements secret 语义和 beta feedback triage。
