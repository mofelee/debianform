...
DebianForm 目前没有 Terraform module system，所以 `variable` 应先定义为 program
...
以下 loop 把本文档拆成可合并闭环，格式对齐 `docs/implementation-plan.zh.md`。每个
...
- [x] 已有 `secrets.file`，并要求 plan/state 不写 secret 明文。
...
- 新增 `internal/core/testassert.NoSecretLeak`，统一检查当前 placeholder secret
  明文不会进入 HostSpec JSON、ResourceGraph desired、plan text/JSON/HTML、state JSON。
- 复用现有 `foundation`、`files-plan-preview`、`component-inputs` fixture，并新增
  `internal/core/testdata/fixtures/sensitive-service-environment.dbf.hcl` 覆盖 structured
...
- 新增 `internal/core/testdata/fixtures/variable-declarations.dbf.hcl` 作为只声明 variable
...
- 新增 `internal/core/testdata/fixtures/variable-defaults.dbf.hcl`，覆盖 host、profile 和
...
- 未声明变量会报错；sensitive variable 的非法 CLI value 会返回脱敏错误，不包含原始
  value。
- 新增 `internal/core/testdata/fixtures/variable-cli.dbf.hcl` 和 CLI smoke，覆盖
...
- sensitive 值的完整 HostSpec、plan 和 state 脱敏传播由 Loop 6 补齐；当前
  `sensitive-variable-files` fixture 和 no-leak 断言已覆盖这条验收项。
...
- 新增 `internal/core/testdata/fixtures/sensitive-variable-files.dbf.hcl`，覆盖直接 file、
...
- 新增 `internal/core/testdata/fixtures/ephemeral-variable-content.dbf.hcl`，覆盖直接
...
- `internal/core/testdata/fixtures/ephemeral-variable-content.dbf.hcl` 已更新为推荐写法：
...
- 新增 `examples/variable-secret-file.dbf.hcl` 展示推荐写法：
