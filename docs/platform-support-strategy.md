...
| Target Debian 13 amd64 | Beta | Primary target; all 24 libvirt cases block merge and release. |
...
| Target Debian 12 amd64 | Beta | Runs the same 24 blocking libvirt cases as Debian 13. |
...
| Target Ubuntu 24.04 LTS amd64 | Preview | Independent blocking 24-case matrix; does not change Debian defaults or Beta tiers. |
...
- It runs exactly the same 24 libvirt cases as Debian 13 amd64; every failure blocks release.
...
- Run complete blocking 24-case matrices separately from Debian. The
...
24-case matrix as `24/24` with its CI run URL and prove all results came from the same release
...
- Debian 13 amd64 is the highest-priority target. Debian 12 amd64 is also Beta and a release
  blocker.
- Ubuntu 24.04 and 26.04 LTS amd64 are Preview, each with an independent blocking 24-case matrix.
- CI runs 96 amd64 libvirt jobs as four targets times 24 cases and requires three aggregate gates.
