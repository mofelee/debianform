# Ubuntu 24.04 目标兼容实施契约

本文档锁定 issue [#43](https://github.com/mofelee/debianform/issues/43) 的实现边界，供
后续 parser、facts、provider、libvirt 和文档改动共同遵守。它在 Ubuntu 完整矩阵通过前属于
Design-only 契约，**不表示当前版本已经支持 Ubuntu**；当前支持状态仍以
[支持矩阵](support-matrix.zh.md)为准。
...
- 不管理 Netplan 或 NetworkManager，不生成 YAML，不调用 `netplan`，也不迁移
  `/etc/netplan/*`。
...
`platform` 增加两个可选 assertion/offline fact 字段，并保留已有字段：
...
| Ubuntu 24.04 amd64 | 实现完成后是 | Preview |
...
Loop 2 必须先运行未适配 baseline，并把真实首个失败阶段记录到 issue。下面只锁定预期 fixture
和责任归类，不可替代实机证据。

| Case | Ubuntu fixture | 预期分类/负责 Loop |
| --- | --- | --- |
...
Loop 6 的最终证据必须在同一提交上给出 Ubuntu 24.04、Debian 12 和 Debian 13 各自的
`20/20` 结果和独立 aggregate gate URL。Preview 表示成熟度，不允许合并已知的 Ubuntu 回归。
