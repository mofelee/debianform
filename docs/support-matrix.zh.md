...
| Target Ubuntu 24.04 LTS amd64 | Preview | 独立 20-case 阻断矩阵；范围和排除项见 Ubuntu 支持契约。 |
| Target Ubuntu 26.04 LTS amd64 | Preview | 官方 released image、独立 20-case 阻断矩阵和独立 gate；只允许 `resolute` amd64 Server。 |
...
CI 当前自动发现 20 个 libvirt cases，并展开为 Debian 12、Debian 13、Ubuntu 24.04 和
Ubuntu 26.04 四套 amd64 矩阵，共 80 个阻断 libvirt jobs。两个 Ubuntu 版本分别使用
...
raw policy route/rule 生效和清理。当前四套完整 20-case 矩阵由上述 CI run 和三个 gates 统一证明。
...
| `test/integration/libvirt/cases/*` | Beta/Preview | Debian 12/13 amd64 各 20 个 Beta cases，Ubuntu 24.04/26.04 amd64 各 20 个 Preview cases；覆盖 platform assertion、validate、online plan、drift check、apply、JSON no-op plan、check 和 case-specific assertions。 |
