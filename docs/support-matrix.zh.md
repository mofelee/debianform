...
目标发行版、版本和架构进入 Beta/Preview 的条件见
...
| Target Debian 13 amd64 | Beta | 最高优先级目标；25 个 libvirt cases 全部作为阻断门禁。 |
| Target Debian 13 arm64 | Preview | runtime facts 和架构选择支持 arm64，但缺少真实 arm64 目标主机矩阵。 |
| Target Debian 12 amd64 | Beta | 25 个 libvirt cases 与 Debian 13 使用同一阻断门禁。 |
...
| Target Ubuntu 24.04 LTS amd64 | Preview | 独立 25-case 阻断矩阵；范围和排除项见 Ubuntu 支持契约。 |
| Target Ubuntu 26.04 LTS amd64 | Preview | 官方 released image、独立 25-case 阻断矩阵和独立 gate；只允许 `resolute` amd64 Server。 |
...
CI 当前自动发现 25 个 libvirt cases，并展开为 Debian 12、Debian 13、Ubuntu 24.04 和
Ubuntu 26.04 四套 amd64 矩阵，共 100 个阻断 libvirt jobs。两个 Ubuntu 版本分别使用
...
加入 `component-moved` case 之前最后一次完整四目标门禁基线是提交
...
[CI run 29418825778](https://github.com/mofelee/debianform/actions/runs/29418825778) 中 Debian 12、
Debian 13、Ubuntu 24.04 和 Ubuntu 26.04 amd64 分别通过 `20/20` 个 libvirt cases，三个
aggregate gates 均成功；连同 unit job 共 `84/84`。Ubuntu 26.04 使用官方 released image
`ubuntu-26.04-server-cloudimg-amd64.img`，SHA-256 为
...
当前 25-case gate 还要求 `component-moved`、`bird-wireguard-networkd`、
`resource-dependencies`、`component-multi-instance-names` 和 `component-artifact-dependency`
在每个目标上通过。
...
| `package.source = "official"` | Beta | 默认值；按已验证 distribution 选择 Docker Debian/Ubuntu 官方 APT 源，安装 `docker-ce`、CLI、containerd、buildx、compose plugin。 |
| `package.repository_url` / `package.gpg_url` | Beta | official source 下覆盖当前发行版的 Docker official APT repo/key URL；自定义 URL 不做跨发行版改写。 |
...
| `examples/ubuntu-24.04-preview.dbf.hcl` | Preview | Ubuntu 24.04 noble amd64 四元组、普通 file apply/no-op/check；不声明网络资源。 |
| `examples/ubuntu-26.04-preview.dbf.hcl` | Preview | Ubuntu 26.04 resolute amd64 四元组、普通 file apply/no-op/check；不声明网络资源。 |
...
| `test/integration/libvirt/cases/bird-wireguard-networkd` | Preview | BIRD-owned dummy/WireGuard topology、reviewed networkd 输出、activation 去重、drift 修复、runtime 删除和脱敏。 |
...
| `test/integration/libvirt/cases/component-multi-instance-names` | Beta | 同一 component 挂载两次，每个实例使用独立的 systemd unit、service、目录和文件名称；no-op 收敛和清理。 |
...
| `test/integration/libvirt/cases/*` | Beta/Preview | Debian 12/13 amd64 各 25 个 Beta cases，Ubuntu 24.04/26.04 amd64 各 25 个 Preview cases；覆盖 platform assertion、validate、online plan、drift check、apply、JSON no-op plan、check 和 case-specific assertions。 |
...
- Ubuntu 22.04、Ubuntu arm64、桌面环境和其他未列出的 Ubuntu 版本或目标系统。
- Netplan、NetworkManager、cloud-init、Snap、PPA 和 Ubuntu Pro 生命周期管理。
