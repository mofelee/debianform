...
| Ubuntu 22.04/26.04、Ubuntu arm64 或桌面 | Unsupported | 不进入当前版本支持承诺或 release gate。 |
...
CI 当前自动发现 20 个 libvirt cases，并展开为 Ubuntu 24.04、Debian 12 和 Debian 13 三套
amd64 矩阵，共 60 个阻断 libvirt jobs。Ubuntu 使用独立 `Ubuntu 24.04 target matrix gate`；
Debian 12/13 使用 `Managed target matrix gate`。Debian 13 仍是本地默认值和新功能的最高
优先级目标。
...
[CI run 29334736664](https://github.com/mofelee/debianform/actions/runs/29334736664) 中 Ubuntu 24.04、
Debian 12 和 Debian 13 amd64 分别通过 `20/20` 个 libvirt cases，两个 aggregate gates 均成功；
连同 unit job 共 `63/63`。
...
raw policy route/rule 生效和清理。当前三套完整 20-case 矩阵由上述 CI run 和两个 gates 统一证明。
...
| `platform` | Beta | 目标 distribution/version/architecture/codename facts，用于目标校验、Docker 官方源、component source selection 和离线 plan。 | merge/graph/plan 单测和三套 target integration matrices。 |
...
| `test/integration/libvirt/cases/*` | Beta/Preview | Debian 12/13 amd64 各 20 个 Beta cases，Ubuntu 24.04 amd64 20 个 Preview cases；覆盖 platform assertion、validate、online plan、drift check、apply、JSON no-op plan、check 和 case-specific assertions。 |
...
- Ubuntu 22.04/26.04、Ubuntu arm64、桌面环境和其他未列出的目标系统。
