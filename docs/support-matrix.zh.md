...
| Target Debian 13 amd64 | Beta | 最高优先级目标；24 个 libvirt cases 全部作为阻断门禁。 |
| Target Debian 13 arm64 | Preview | runtime facts 和架构选择支持 arm64，但缺少真实 arm64 目标主机矩阵。 |
| Target Debian 12 amd64 | Beta | 24 个 libvirt cases 与 Debian 13 使用同一阻断门禁。 |
...
| Target Ubuntu 24.04 LTS amd64 | Preview | 独立 24-case 阻断矩阵；范围和排除项见 Ubuntu 支持契约。 |
| Target Ubuntu 26.04 LTS amd64 | Preview | 官方 released image、独立 24-case 阻断矩阵和独立 gate；只允许 `resolute` amd64 Server。 |
...
CI 当前自动发现 24 个 libvirt cases，并展开为 Debian 12、Debian 13、Ubuntu 24.04 和
Ubuntu 26.04 四套 amd64 矩阵，共 96 个阻断 libvirt jobs。两个 Ubuntu 版本分别使用
...
当前 24-case gate 还要求 `component-moved`、`bird-wireguard-networkd`、
`resource-dependencies` 和 `component-multi-instance-names` 在每个目标上通过。
...
| `test/integration/libvirt/cases/*` | Beta/Preview | Debian 12/13 amd64 各 24 个 Beta cases，Ubuntu 24.04/26.04 amd64 各 24 个 Preview cases；覆盖 platform assertion、validate、online plan、drift check、apply、JSON no-op plan、check 和 case-specific assertions。 |
