...
Debian 12 和 Debian 13 的证据不能合并成一句“libvirt passed”。当前 20-case 矩阵必须分别记录
`20/20` 和 CI run URL，并确认两者来自 release commit；aggregate gate 也必须单独记录结果。
...
- CI 当前按 2 个版本 x 20 个 cases 运行 40 个 amd64 libvirt jobs。
