...
amd64 同为 Beta；Ubuntu 24.04 和 26.04 LTS amd64 以各自独立门禁进入 Preview。
...
| Target Debian 13 amd64 | Beta | 当前主目标；25 个 libvirt cases 全部阻断合并和发布。 |
| Target Debian 13 arm64 | Preview | runtime facts 和 artifact source selection 支持 arm64，但缺少真实目标矩阵。 |
| Target Debian 12 amd64 | Beta | 与 Debian 13 运行相同的 25 个阻断式 libvirt cases。 |
...
| Target Ubuntu 24.04 LTS amd64 | Preview | 独立 25-case 阻断矩阵；不改变 Debian 默认和 Beta 等级。 |
| Target Ubuntu 26.04 LTS amd64 | Preview | 官方 released image、独立 25-case 阻断矩阵和 `resolute` 精确 tuple。 |
...
- 运行与 Debian 13 amd64 完全相同的 25 个 libvirt cases，任何失败都是 release blocker。
...
## Ubuntu LTS Preview 策略

Ubuntu 24.04 LTS (`noble`) 和 Ubuntu 26.04 LTS (`resolute`) amd64 的 Preview 共享产品边界，
但必须保留独立证据：

- 使用同一个 `dbf` CLI、DSL、resource address、plan 格式和 state schema，不创建 UbuntuForm。
- 继续只支持 root SSH，不支持 sudo/become 或默认 `ubuntu` 用户连接。
- 与 Debian 分开运行完整 25-case 阻断矩阵；24.04 和 26.04 分别由
...
- Ubuntu 22.04、其他 Ubuntu 版本、arm64、桌面、Snap、PPA、Ubuntu Pro、cloud-init 生命周期
  和 24.04 到 26.04 原地升级不在支持范围。

完整契约见 [Ubuntu 24.04 支持契约](ubuntu-24.04-support-contract.zh.md)与
[Ubuntu 26.04 支持契约](ubuntu-26.04-support-contract.zh.md)。26.04 的 released-image 基线为
`ubuntu-26.04-server-cloudimg-amd64.img`，SHA-256
`0826c5005ebc70edcfc4519e5d65eca766782f16426231c4c3e92b811ba8df0b`。提交
...
的 [CI run 29418825778](https://github.com/mofelee/debianform/actions/runs/29418825778) 证明四个
目标各 `21/21` 且三个 aggregate gates 成功。Preview 提升到 Beta 不能只依据一次绿色矩阵；
还要求持续阻断 CI、release 验证、无未解决高风险反馈和显式支持等级决策。
...
- 当前项目资源优先维护四套 amd64 目标矩阵和 Debian 13 主路径。
...
- Ubuntu 24.04 和 26.04 amd64 各有独立 release-blocking Preview gate。
...
- Ubuntu 26.04 amd64 被管理目标主机 integration。
- `Managed target matrix gate`、`Ubuntu 24.04 target matrix gate` 和
  `Ubuntu 26.04 target matrix gate`。

四个目标的证据不能合并成一句“libvirt passed”。当前 25-case 矩阵必须分别记录 `25/25`
...
- Ubuntu 24.04 和 26.04 LTS amd64 为 Preview，各使用独立 25-case 阻断矩阵。
- CI 当前按 4 个目标 x 25 个 cases 运行 100 个 amd64 libvirt jobs，并要求三个 aggregate gates。
