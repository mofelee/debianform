...
| Target Ubuntu 26.04 LTS amd64 | Preview | 官方 released image、独立 20-case 阻断矩阵和 `resolute` 精确 tuple。 |
...
- 与 Debian 分开运行完整 20-case 阻断矩阵；24.04 和 26.04 分别由
...
目标各 `20/20` 且三个 aggregate gates 成功。Preview 提升到 Beta 不能只依据一次绿色矩阵；
...
四个目标的证据不能合并成一句“libvirt passed”。当前 20-case 矩阵必须分别记录 `20/20`
...
- Ubuntu 24.04 和 26.04 LTS amd64 为 Preview，各使用独立 20-case 阻断矩阵。
- CI 当前按 4 个目标 x 20 个 cases 运行 80 个 amd64 libvirt jobs，并要求三个 aggregate gates。
