...
- [x] `examples/v2-nftables.dbf.hcl` 进入 golden 测试
- [x] `examples/v2-plan-preview.dbf.hcl` 作为 nftables + secret 完整 plan preview fixture
...
- [x] 移除旧实验入口和代码路径
- [x] 清理 v2 中不再需要的临时 feature gate；仅保留 validate/fmt/runtime facts bootstrap 的内部编译选项
- [x] 错误信息统一使用 v2 用户术语
- [x] release build 包含 v2 文档和示例

测试：

- [x] 所有第一版 v2 examples 进入 validate 测试（design-only fixture 除外）
- [x] 小型 golden examples 覆盖 parser、HostSpec、ResourceGraph、plan
- [x] `dbf fmt` 幂等性测试
- [x] `v2-fleet` 只作为组合压力测试
...
以下 loop 把 `docs/v2-component-input-requirements.zh.md` 拆成可合并闭环。它们属于 v2
第一版之后的增强，不改变上面的第一版验收结论。

总体原则：

- [ ] 每个 loop 都必须保持现有 `type/default/sensitive` input 配置兼容
- [ ] 每个 loop 都必须有 parser 单测、merge/compiler 单测和至少一个 golden 覆盖
- [ ] 新增复杂 input 能力必须先通过 `dbf validate`，再进入 plan/apply 路径
- [ ] 敏感数据不能因为新类型系统进入 HostSpec JSON、plan JSON 或 state JSON 明文
- [ ] `make test` 必须通过
...
- [ ] 新增 component input type parser，基于 HCL AST 解析类型表达式
- [ ] 新增机器可读 `ComponentInputTypeSpec`，不要只保存 canonical string
- [ ] `parser.ComponentInput` 增加 `TypeSpec`、`TypeExpr`、`Description`、`Nullable`
- [ ] `ir.ComponentInputSpec` 增加 `TypeSpec`、`TypeExpr`、`Description`、`Nullable`
- [ ] `parseComponentInputBlock` 支持 `description` 和 `nullable`
- [ ] 保持旧 `Type string` JSON/调试输出兼容，值为 canonical type expression
- [ ] 实现 default 值按 type conversion 和 optional default 归一化
- [ ] 实现 instance input 值按 type conversion 和 optional default 归一化
- [ ] `nullable = false` 拒绝顶层 null
- [ ] `object` 缺失必填字段时报错
- [ ] `object` 多余字段默认报错
- [ ] `tuple` 长度不一致时报错
- [ ] `set(T)` 输出使用确定性顺序，保证 HostSpec/golden 稳定
- [ ] `array(T)` 明确报错，并提示使用 `list(T)`
- [ ] component body 展开时注入归一化后的结构化 `input`
- [ ] 表达式 evaluator 增加 `jsonencode`，用于把结构化 input 写入文件内容
- [ ] 错误 path 能定位到嵌套字段和 list 下标

测试：

- [ ] parser 单测覆盖 primitive、`any`、`list(T)`、`set(T)`、`map(T)`、`tuple([...])`
- [ ] parser 单测覆盖 `object({ name = string, enabled = optional(bool, true) })`
- [ ] parser 单测覆盖 `list(object({ ... }))`
- [ ] parser 负例覆盖 `array(string)`、裸 `list`、裸 `map`、object 外的 `optional(...)`
- [ ] merge 单测覆盖 default 归一化
- [ ] merge 单测覆盖 instance input 归一化
- [ ] merge 负例覆盖缺失 object 必填字段
- [ ] merge 负例覆盖 object 多余字段
- [ ] merge 负例覆盖嵌套字段类型错误，并断言 source path 含下标
- [ ] HostSpec golden 覆盖 `list(object(...))` 归一化结果
- [ ] ResourceGraph/plan golden 覆盖结构化 input 生成文件内容

示例：

- [ ] 新增 `examples/v2-component-inputs.dbf.hcl`
- [ ] 示例 component 使用 `list(object(...))` 生成 JSON 配置文件
- [ ] 示例覆盖 optional 字段默认值补齐
- [ ] 示例进入 runnable v2 examples validate 测试

文档：

- [ ] README 增加 component input 2.0 简短示例
- [ ] `docs/v2-component-input-requirements.zh.md` 标记本轮已实现范围
- [ ] `docs/v2-requirements.md` 更新 component input 支持类型列表
- [ ] `docs/v2-ir-requirements.zh.md` 更新 ComponentInputSpec

验收：

- [ ] `dbf validate -f examples/v2-component-inputs.dbf.hcl` 成功
- [ ] `dbf plan -f examples/v2-component-inputs.dbf.hcl --offline --format json` 成功
- [ ] HostSpec 中 `list(object(...))` 已归一化并稳定排序
- [ ] `make test` 成功
...
- [ ] `parser.ComponentInput` 增加 `Validations []ComponentInputValidation`
- [ ] `ir.ComponentInputSpec` 增加 validation metadata
- [ ] `parseComponentInputBlock` 允许 nested `validation` block
- [ ] validation block 必须无 label
- [ ] validation block 必须包含 `condition` 和 `error_message`
- [ ] `error_message` 必须是非空字符串
- [ ] validation 在 default、type conversion、optional default、nullable 检查之后执行
- [ ] validation condition 必须求值为 bool
- [ ] validation 只能读取当前 input：`input.<name>`
- [ ] 禁止 validation 读取其他 input、`target`、`local`、`path`
- [ ] 增加 validation 专用纯函数集合：`length`、`contains`、`startswith`、`endswith`
- [ ] 增加集合函数：`alltrue`、`anytrue`、`distinct`、`sort`、`keys`、`values`
- [ ] 增加转换函数：`tonumber`、`tostring`、`tobool`
- [ ] 增加 `regex` 和 `can`
- [ ] validation 失败时阻止 validate/plan/apply，并指向 validation source

测试：

- [ ] parser 单测覆盖重复 validation block
- [ ] parser 负例覆盖 validation label、缺少 condition、缺少 error_message、空 message
- [ ] merge 单测覆盖 validation 成功
- [ ] merge 单测覆盖 validation 失败
- [ ] merge 单测覆盖 condition 非 bool
- [ ] merge 负例覆盖读取其他 input
- [ ] merge 负例覆盖读取 `target`、`local`、`path`
- [ ] 函数单测覆盖 `alltrue`、`anytrue`、`regex`、`can`
- [ ] CLI smoke 覆盖 `dbf validate` 输出 validation failure source path

示例：

- [ ] 更新 `examples/v2-component-inputs.dbf.hcl`，增加端口范围 validation
- [ ] 增加 invalid fixture 覆盖 validation failure

文档：

- [ ] README 示例展示一个短 validation
- [ ] `docs/v2-component-input-requirements.zh.md` 标记 validation 已实现函数集合
- [ ] 常见错误增加 validation failure 示例

验收：

- [ ] 合法 input validation 通过
- [ ] 非法 port validation 报错并指向 `component.<name>.input["..."].validation[0]`
- [ ] validation 不能访问 `target.system.codename`
- [ ] `make test` 成功
...
- [ ] 在 input 注入 cty value 时为 sensitive input 加 mark
- [ ] `ctyToValue` 保留 sensitive mark
- [ ] `parser.Value` 或等价 wrapper 增加 sensitive metadata
- [ ] 表达式求值后聚合 sensitive mark
- [ ] `parserValueToAny` 对 sensitive value 输出 `"<sensitive>"`
- [ ] HostSpec JSON 中 component `input_values` 对 sensitive 值 redacted
- [ ] 由 sensitive value 生成的 `files.file.content` 自动视为 sensitive
- [ ] 由 sensitive value 生成的 `systemd.unit.content` 自动视为 sensitive
- [ ] 由 sensitive value 生成的 service environment 自动视为 sensitive
- [ ] plan text/JSON 对 sensitive content 只输出 hash、长度等摘要
- [ ] state JSON 不保存 sensitive content 明文
- [ ] 如果某类资源暂不能安全承载 sensitive 派生值，编译时必须报错

测试：

- [ ] unit test 覆盖 sensitive input value redaction
- [ ] unit test 覆盖 sensitive object 内部字段 redaction
- [ ] unit test 覆盖 sensitive input 派生 file content
- [ ] unit test 覆盖 sensitive input 派生 systemd unit content
- [ ] plan golden 确认 sensitive 明文不出现
- [ ] state 单测确认 sensitive 明文不出现
- [ ] 负例覆盖 sensitive 派生值进入暂不支持的非敏感字段时失败

示例：

- [ ] 更新 `examples/v2-component-inputs.dbf.hcl`，增加 sensitive `environment` input
- [ ] 示例 plan golden 确认 secret value 不泄漏

文档：

- [ ] README 补充 sensitive input 行为说明
- [ ] `docs/v2-state.md` 补充 sensitive input 派生内容不落盘规则
- [ ] `docs/v2-plan-format.md` 补充 sensitive summary/redaction 字段
- [ ] `docs/v2-component-input-requirements.zh.md` 标记 sensitive propagation 已实现范围

验收：

- [ ] `dbf plan -f examples/v2-component-inputs.dbf.hcl --offline --format json` 不包含 sensitive 明文
- [ ] state 序列化测试不包含 sensitive 明文
- [ ] `make test` 成功
...
- [ ] `parseComponentInputBlock` 支持 `deprecated`
- [ ] `parser.ComponentInput` 和 `ir.ComponentInputSpec` 增加 `Deprecated`
- [ ] 调用方显式传入 deprecated input 时产生 warning
- [ ] 仅使用 deprecated input 的 default 时不产生 warning
- [ ] CLI warning 统一聚合，`validate`、`plan`、`apply` 输出格式一致
- [ ] warning 不改变退出码
- [ ] 新增 `dbf component inspect -f <file> <component>`，输出 input 名称、类型、默认值、nullable、sensitive、deprecated 和 description
- [ ] inspect 输出 redacts sensitive default
- [ ] inspect 对 unknown component 返回明确错误
- [ ] release build 包含新增文档和示例

测试：

- [ ] parser 单测覆盖 `deprecated`
- [ ] merge 单测覆盖显式传 deprecated input 产生 warning
- [ ] merge 单测覆盖 default 不产生 deprecated warning
- [ ] CLI smoke 覆盖 validate warning 输出
- [ ] CLI smoke 覆盖 plan warning 输出
- [ ] CLI smoke 覆盖 `dbf component inspect`
- [ ] inspect golden 覆盖复杂 `list(object(...))` 类型显示
- [ ] libvirt 集成测试覆盖 component input 2.0 生成实际文件

示例：

- [ ] `examples/v2-component-inputs.dbf.hcl` 进入 README 示例列表
- [ ] 新增 `test/integration/libvirt/cases/component-inputs`
- [ ] 集成测试验证 optional default 后的文件内容
- [ ] 集成测试验证 sensitive value 未进入远端 state 文件

文档：

- [ ] README 增加 component input 2.0 完整入口
- [ ] README 常见错误补充 object 字段类型错误和 validation 失败
- [ ] `docs/v2-component-input-requirements.zh.md` 更新实现状态
- [ ] `docs/v2-implementation-plan.zh.md` 勾选完成项

验收：

- [ ] `dbf component inspect -f examples/v2-component-inputs.dbf.hcl reverse_proxy` 输出完整 input API
- [ ] `dbf validate -f examples/v2-component-inputs.dbf.hcl` 成功且 warning 行为符合预期
- [ ] `make test` 成功
