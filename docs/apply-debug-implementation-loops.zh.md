...
- [x] 调整 `cmd/dbf/main.go` 的 flag 校验，让 `--debug` 同时支持 `plan` 和 `apply`。
- [x] 在 apply 分支增加 debug mode 选项传递结构，但不改变普通 apply 行为。
- [x] 调试模式下强制 `parallel <= 1`。
- [x] usage 文本更新为 `dbf apply ... [--debug]`。

测试：

- [x] CLI 测试覆盖 `apply --debug` 可解析。
- [x] CLI 测试覆盖 `plan --debug` 仍输出 `provider_address`。
- [x] CLI 测试覆盖 `check --debug`、`validate --debug` 报错。
- [x] CLI 测试覆盖 `apply --debug --parallel 2` 报错。
...
## Loop 2: DebugRunner 骨架和非交互记录模式（已完成）
...
- [x] 新增 `internal/core/engine/debug_runner.go` 或等价文件。
- [x] 新增 `DebugSession`，负责序号、输出 writer、结果格式化。
- [x] 实现短文本 payload/stdout/stderr 的完整展示。
- [x] 空 stdout/stderr 显示 `<empty>`。
- [x] 为 `RunInput` 保留 payload bytes，确保执行和后续展示一致。

测试：

- [x] fake runner 单测覆盖 `Run`、`RunCommand`、`RunInput` 都被转发。
- [x] 单测断言输出包含 host、runner、script/command、stdout、stderr。
- [x] 单测断言 `RunInput` 的 payload 被传给 inner runner 且可打印。
- [x] 单测断言错误会返回原错误，并打印 failed 结果。
...
- [x] 增加 payload 分类函数：短文本、长文本、二进制。
- [x] 增加 sha256、byte length、line count 计算。
- [x] 增加 preview 生成逻辑，建议阈值为 8 KiB 或 200 行。
- [x] 增加完整展开渲染函数，二进制输出 hex dump。

测试：

- [x] 短文本默认完整显示。
- [x] 超过阈值的长文本默认只显示摘要和 preview。
- [x] 非 UTF-8 payload 默认只显示 binary 摘要。
- [x] `show stdin` 的渲染函数可输出完整文本或完整 hex dump。
- [x] stdout/stderr 使用同一策略。
...
## Loop 4: 交互 prompt、step/next/continue/show/quit（已完成）
...
- [x] `DebugSession` 增加 input reader。
- [x] 实现命令解析和状态机。
- [x] `BeforeCall` 根据当前模式决定是否等待 prompt。
- [x] `continue` 模式仍打印调用详情和结果，但不等待输入。
- [x] `quit` 返回明确取消错误，例如 `apply cancelled by debugger`。

测试：

- [x] fake stdin 输入 `step`，只执行一个调用后下一次继续等待。
- [x] fake stdin 输入 `next 5`，连续放行 5 个调用。
- [x] fake stdin 输入 `continue`，后续调用不再等待输入。
- [x] fake stdin 输入 `show stdin`，展开当前 payload 但不执行调用。
- [x] fake stdin 输入 `quit`，返回取消错误且不执行当前调用。
- [x] 未知命令不执行调用，并打印帮助提示。
...
- [x] 在 CLI apply 分支创建 `DebugSession`。
- [x] 用 `DebugRunner` 包装 `SSHRunner`，同时保留底层 `SSHRunner.Close`。
- [x] 调试模式传入 `factsParallel=1`。
- [x] 调试模式传入 `coreengine.Options{Parallel: 1, PerHostParallel: 1}`。
- [x] 确保普通 `apply` 未启用 debug 时无行为变化。

测试：

- [x] CLI/fake runner 测试验证 `apply --debug` 的 stderr 出现调试 warning。
- [x] 测试验证 debug 模式下 stdout 仍是 plan/apply 正文。
- [x] 测试验证 debug 模式设置并发为 1。
- [x] 测试验证普通 apply 不输出调试内容。
...
- [x] 新增 `RemoteCallContext` 和 `WithRemoteCallContext` / getter。
- [x] 在 facts、backend、engine execute、provider plan/apply 关键入口注入上下文。
- [x] `DebugRunner` 从 context 读取并打印 phase/address/action/summary。
- [x] state unlock 设置 cleanup 标志，为下一轮自动放行准备。

测试：

- [x] 单测覆盖 context 注入后调试输出包含 phase/address/action/summary。
- [x] 单测覆盖无 context 时 fallback 为 `remote call`。
- [x] 单测覆盖 state read/write/lock/unlock phase。
- [x] 单测覆盖 apply resource 和 operation phase。
...
## Loop 7: cleanup 自动执行和失败 retry（已完成）
...
- [x] `DebugRunner` 对失败调用保存 last call 和 last result。
- [x] 失败后进入 retry loop。
- [x] `RunInput` retry 使用内存中的同一 payload。
- [x] cleanup 调用绕过 prompt，但仍打印详情和结果。
- [x] quit/cancel 后 cleanup 仍可执行。

测试：

- [x] fake runner 第一次失败、retry 成功。
- [x] fake runner retry 仍使用同一 stdin payload。
- [x] 失败状态输入 `step` / `next` / `continue` 不执行调用。
- [x] quit 后普通调用不执行，cleanup 调用仍执行。
- [x] cleanup 调用不等待 prompt。
...
- [x] `docs/cli.zh.md` 记录 `apply --debug`。
- [x] `docs/operations-runbook.zh.md` 增加调试 apply 的排障用法。
- [x] `docs/how-it-works/08-apply-scheduler.zh.md` 补充 debug runner 在 apply 流程中的位置。
- [x] 如果需要，README 或用户手册增加一段简短入口说明。

测试：

- [x] `make test`。
- [x] `go vet ./...`。
- [x] 若环境允许，选一个 libvirt case 用 `--debug --auto-approve` 和 `continue` 跑通。
- [x] 手工验证长文本和二进制 payload 不自动刷屏，`show stdin` 可展开。
