...
升级 `examples/v2-wireguard-networkd.dbf.hcl`：
...
`examples/v2-wireguard-networkd.dbf.hcl` 和
`examples/v2-systemd-networkd-wireguard.dbf.hcl` 原先内容完全相同。保留
`examples/v2-wireguard-networkd.dbf.hcl` 作为唯一可运行 WireGuard component 示例，
