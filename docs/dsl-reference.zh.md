...
| `on_change` | 无 | 仅 component 内可用，值必须是同 component 内的 `script.<name>` 引用。当前版本会解析并编译到 HostSpec；触发 operation 和执行在后续版本实现。 |
...
<!-- dbf-test:name=component-script-on-change-syntax;commands=validate -->
...
当前实现范围是 DSL 解析、validate 和 HostSpec 编译；ResourceGraph operation 生成、
