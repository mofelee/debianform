...
- `Write(ctx, host, st)`：写入 host state，并在成功时返回 committed state。
...
1. `state.PrepareWrite(st, host.Name)` 在严格归一化的副本上把 `serial` 增加 1，并更新
   `updated_at`，得到尚未提交的 candidate；MemoryBackend 也遵循同一准备契约。
2. `state.Encode(candidate)` 校验并生成 JSON；Encode 本身不改变 serial 或时间。
3. base64 编码 JSON，避免 shell heredoc 内容转义问题。
4. 远端创建 state 目录。
5. 写到临时文件 `state.path.tmp`。
6. `mv tmp state.path` 原子替换。
7. 只有远端写入成功后才把 candidate 作为 committed state 返回。

engine 只在 `Write` 成功后采用返回的 committed state。远端命令、编码或校验失败时，candidate
会被丢弃，调用方仍持有原 serial；用原 state 重试只会得到 `N+1`。MemoryBackend 在保存和返回
committed state 时使用副本，确保 memory 与 SSH backend 的 revision 契约一致。
