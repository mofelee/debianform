...
- libvirt integration：守护真实 Debian 主机行为。
...
`internal/v2/engine/*_test.go` 适合覆盖：
...
`test/integration/libvirt` 覆盖真实 Debian 主机上的 apply/check 流程。case 通常包含：
