...
`apply` 先生成一份未持锁的在线 preview plan。确认后，它会获取远端 state lock、重新读取 state 和
observed 状态，再打印真正要执行的 locked plan，之后才会写 state 或修改远端主机。state lock 避免同一
...
如果持锁后的实际计划与已确认 preview 相同，会复用这次确认；如果不同，会先打印新计划和变化提示，
再要求确认一次。拒绝第二次确认会释放 lock，不写 facts/resource state，也不调用 provider 修改主机。
...
| `--auto-approve` | `false` | 跳过两次计划的交互确认；持锁后的实际执行计划仍会打印。 |
| `--debug` | `false` | 进入交互式 SSH 调试器。只能用于 `apply`；会强制串行，不能和 `--parallel` 大于 1 同时使用。 |
| `-var name=value` | 空 | 可重复；设置顶层变量。 |
| `-var-file path` | 空 | 可重复；加载变量文件。 |

如果 preview plan 没有任何变更和 operation，`apply` 会输出 plan 后直接结束，不会获取 apply lock 或执行确认步骤。
每台 host 内部仍按 ResourceGraph 的确定性顺序串行执行。
进度日志不会输出资源 desired 内容或远端命令 stdout，因此不会破坏 JSON/text plan 消费。

## check

`check` 生成在线 plan 并用于检测远端状态是否偏离配置。开始检查前，它会先获取所有目标 host 的
state lock，并持有这些锁直到 state 读取和全部 provider 观测结束；`--lock-timeout` 控制获取每个锁
时的最长等待时间。这样 check 不会和其他遵守同一锁的 DebianForm 操作交错读写 state 与受管资源。

`check` 是只读操作：它不会写入 state、持久化 host facts、应用或删除资源，也不会运行 operation。
配置解析所需的 runtime facts discovery 发生在这个锁周期之前，外部进程也不受 DebianForm state lock
约束。探测进度会写到 stderr，检查结果 plan 写到 stdout。
...
| `--lock-timeout duration` | `5m` | 等待远端 state lock 的最长时间；锁会持有到整个检查结束。 |
| `-var name=value` | 空 | 可重复；设置顶层变量。 |
| `-var-file path` | 空 | 可重复；加载变量文件。 |
