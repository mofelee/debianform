...
func (b *MemoryBackend) Read(ctx context.Context, host ir.HostSpec) (v2state.State, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *MemoryBackend) Write(ctx context.Context, host ir.HostSpec, st v2state.State) error {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (p *MemoryProvider) Plan(ctx context.Context, node graph.Node, prior *v2state.Resource) (ProviderPlan, error) {
	p.mu.Lock()
	defer p.mu.Unlock()
