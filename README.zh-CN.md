...
DebianForm 是一个以 Debian 为主、兼容受支持 Ubuntu 目标的声明式配置工具：写一份 `.dbf.hcl`，先看
`plan`，再执行 `apply`，最后用 `check` 检查漂移。

> 管理 Alpine Linux？请查看姊妹项目 [AlpineForm](https://github.com/mofelee/alpineform)。
...
当前项目仍处于 public preview / beta 阶段。Debian 13 amd64 是最高优先级目标，Debian 12
amd64 也进入 Beta 支持；Ubuntu 24.04 和 26.04 LTS amd64 是分别门禁的 Preview 目标。建议先在
低风险测试主机上试用。CLI、配置格式、state 和 plan JSON 在 stable 前仍可能调整。
...
更完整的 Debian 新手教程见 [Quickstart](docs/quickstart.zh.md)；Ubuntu Preview 请按目标版本从
[24.04 Quickstart](docs/ubuntu-24.04-quickstart.zh.md)或
[26.04 Quickstart](docs/ubuntu-26.04-quickstart.zh.md)开始。后续章节见
...
`dbf plan`；Ubuntu 纯离线预览必须声明完整的 `platform.distribution`、`platform.version`、
`platform.architecture` 和 `platform.codename`。
...
- [Quickstart](docs/quickstart.zh.md)：Debian 上从零到第一次 `apply/check`。
- [Ubuntu 24.04 Preview Quickstart](docs/ubuntu-24.04-quickstart.zh.md)：Ubuntu 24.04 LTS amd64 的安全首跑路径。
- [Ubuntu 26.04 Preview Quickstart](docs/ubuntu-26.04-quickstart.zh.md)：Ubuntu 26.04 LTS amd64 与 `resolute` 完整 tuple 的安全首跑路径。
...
- [Plan JSON 格式](docs/plan-format.zh.md)：`dbf plan --format json` 的结构化输出。
- [State 格式](docs/state.zh.md)：远端 state、lock、ownership 和脱敏规则。
